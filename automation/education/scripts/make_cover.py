...
def make_cover(title, output_path, font_path, video_path=None):
    ...
    font_mid = ImageFont.truetype(font_path, 56) if os.path.exists(font_path) else ImageFont.load_default()
    ...
    draw.rounded_rectangle([(60, 80), (60 + tag_w, 80 + tag_h)],
                           radius=tag_h // 2, fill=(255, 69, 0))
    draw.text((80, 80 + tag_h // 2), tag_text, fill="white", font=font_small, anchor="lm")

    # 5. 艺术字标题
    title_lines = _wrap_text(title, font_big, W - 160)
    title_y = H // 2 - len(title_lines) * 70
    for line in title_lines:
        ...
        draw.text((W // 2, title_y), line, fill="#FFD700", font=font_big, anchor="mm")
        title_y += 140

    # 6. 底部CTA
    draw.text((W // 2, H - 120), "🏆 想考消防证？加我咨询详情", fill=(255, 255, 255, 200), font=font_mid, anchor="mm")
